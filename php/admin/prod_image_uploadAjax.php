...
header('charset=UTF-8');
	...
	if ($_FILES['filename1']['name']) {
		$filename = upload($_FILES['filename1'],2000*1024*1024,'product');
...
if ($filename) {
    if($_REQUEST["isEdit"] == "Y"){
     // 기존에 업로드 된 파일이 있으면 삭제합니다.
        $SQL = " Select imageurl from db_product ";
        $SQL .= " where boardSerno = '$boardSerno'";
    //echo $SQL ."<BR>";
        ...
        if( $result === false) {
            ...
            $photo = $row["imageurl"];
            del_file($photo,'product');
    
         mysqli_free_result($result);
        }

        $SQL = "UPDATE db_product SET ";
        $SQL .= " imageurl = '$filename'";
        $SQL .= " where  boardSerno = '$boardSerno'";
        $result = mysqli_query($kiki_conn, $SQL);
        if ( $result === false ) {
           die( print_r( mysqli_connect_error(), true));
