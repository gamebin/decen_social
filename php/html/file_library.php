...
//	$file_name_db = $file_name . '||' . $file_name; //db에 저장될 화일명 예) 새파일명||새파일명
/**/	$re_ext = strtolower($ext);
	$sumFile = $save_dir.'/'.$folder1.'/thumb_'.$file_name;

//echo "temp = ". $file['tmp_name'] . " <BR>";
/**/ if ($re_ext == "gif") {  // thumb 처리
		thumbnail(1,$file['tmp_name'],$sumFile,"410","230");
	} elseif ($re_ext == "jpg" || $re_ext == "jpeg") {
		thumbnail(2,$file['tmp_name'],$sumFile,"410","230");
	} elseif ($re_ext == "png") {
		thumbnail(3,$file['tmp_name'],$sumFile,"410","230");
	} elseif ($re_ext == "bmp") {
		thumbnail(4,$file['tmp_name'],$sumFile,"410","230");
	}
    //화일을 지정된 폴더로 이동시킨다.
...
//echo "file = $file / folder = $folder1 / original_file_name = $original_file_name <BR>";
//exit;
...
function download_nofolder($file) {
    global $save_dir;
...
$file = iconv("UTF-8", "EUC-KR", $file); // utf-8 을 euc-kr 로 변환
    ...
    $HTTP_USER_AGENT = $_SERVER['HTTP_USER_AGENT'];
echo "file = $file / original_file_name = $original_file_name <BR>";
//exit;
    if( preg_match( "#\.\./|\/\/#", $save_file_name ) ) MsgBox("다운로드 과정에서 에러가 발생하였습니다.");
    if(strstr($HTTP_USER_AGENT, "MSIE 6.")) { 
        header("Content-type: application/octetstream"); 
...
function upload2(&$file,$numb,$limit_file_size, $folder1) {
    ...
    $file_name = $new_file_name . '.' . $ext; //파일 이름뒤에 확장자를 붙인다.
	$file_name_db = $file_name . '||' . $file_name;
...
//echo "temp = ". $file['tmp_name'] . " <BR>";
/**/ if ($re_ext == "gif") {  // thumb 처리
		thumbnail(1,$file['tmp_name'],$sumFile,"125","104");
	} elseif ($re_ext == "jpg" || $re_ext == "jpeg") {
		thumbnail(2,$file['tmp_name'],$sumFile,"125","104");
	} elseif ($re_ext == "png") {
		thumbnail(3,$file['tmp_name'],$sumFile,"125","104");
	} elseif ($re_ext == "bmp") {
		thumbnail(4,$file['tmp_name'],$sumFile,"125","104");
	}
...
function upload4($file_name, $file_size,$tmp_name,$numb,$limit_file_size, $folder1) {
    global $save_dir;
    //금지된 확장자 설정 - 금지할 확장자를 추가해서 사용
    $ban_ext = array('php','php3','html','htm','cgi','pl','asp','jsp','exe','com');
//--	$size = $file[size];
	...
	$ext = strtolower($temp_name[sizeof($temp_name)-1]);
	$temp_name2 = str_replace("&","",$file_name);
    if (in_array($ext,$ban_ext)) {
        MsgBox("업로드가 불가능한 확장자입니다.");
    }
    ...
    $file_name = $new_file_name . '.' . $ext; //파일 이름뒤에 확장자를 붙인다.
    $file_name_db = $file_name ; //db에 저장될 화일명 예) 새파일명

    //화일을 지정된 폴더로 이동시킨다.
	if(move_uploaded_file($tmp_name,$save_dir.'/'.$folder1.'/'.$file_name)) {
        @unlink($tmp_name);
        return $file_name_db;
    } else {
        @unlink($tmp_name);
