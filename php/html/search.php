...
<head>
  ...
  <title>검색 - Linking: Decentralized Social Networking</title>
...
<?	}
	...
	$SQL = "Select count(userSerno) as totcnt from db_user a where $wheStr  ";
  ...
  <main role="main" class="container">
    ...
    <div class="row mt-3 mt-lg-5">
      ...
      <div class="col-12">
        ...
        <h5><span class="text-primary"><?=$schStr?></span>으로 검색한 결과입니다. <span class="badge badge-primary badge-pill"><?=$totcnt?></span></h5>
		<div id="pro_con01">
<!-- Ajax 내용 보이기 -->
		</div>
...
function go_follow(userid) {
	$.ajax({
        url: './search_followingAjax.php?callback=?',
        type: 'POST',
		data: {
		  "userid": userid,
       },
        ...
        success: function (data) {
		  if(data.prog == "true"){
			$("#btn_"+userid).removeClass('ml-auto btn btn-outline-secondary');
			$("#btn_"+userid).attr('class','ml-auto btn btn-primary');
			$("#btn_"+userid).text('팔로우');
		  } else {
			  alert(data.msg);
		  }
	    },
